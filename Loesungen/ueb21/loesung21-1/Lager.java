...
// neu in ueb21-streams
import java.util.stream.Collectors;                        // NEU NEU
import java.util.stream.Stream;                            // NEU NEU
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 ...
 *    Artikel.
 *
 * @version 2.01 Beta 10.01.2022 erweitert 02.06.2022 + 21.06.2022 + 30.06.2022
...
//------------------- abgang buchen  --------------------------------
  ...
  public void aenderePreisAllerArtikel ( double prozent )
  {
    ...
    lager.values().stream().forEach((Artikel a) -> a.aenderePreis( prozent ));  //NEU NEU
  }
...

 ...
 //vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
   ...
   */
  // das Original aus ueb18 vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
  ...
  public List<Artikel> getSorted( Comparator<Artikel> c )
    {
      return lager.values().stream().sorted( c ).collect( Collectors.toList() );   //NEU NEU
    }
  ...
  public void applyToArticles(Consumer<Artikel> consum)
    {
      lager.values().stream().forEach( consum ); //NEU NEU
    }
  ...
  public List<Artikel> filter(Predicate<Artikel> filterKrit)
    {
       return lager.values().stream().filter( filterKrit ).collect( Collectors.toList() );   //NEU NEU
    }
  ...
  public void applyToSomeArticles(Predicate<Artikel> f, Consumer<Artikel> c)
    {
      lager.values().stream().filter( f ).forEach( c );            //NEU NEU
    }
  ...
  //  return result;
  //}
  public List<Artikel> getArticles(Predicate<Artikel> f, Comparator<Artikel> c)               //NEU NEU
    {
      return lager.values().stream().filter( f ).sorted( c ).collect( Collectors.toList() );  //NEU NEU
    }
   ...
   * @return eine Liste alle Artikel im Lager, die alle Filterkriterien erfüllen. 
   ...
   public Artikel[]  filterAll(Predicate<Artikel>... fKriterien)
     {
       Collection<Artikel> zuFiltern = lager.values();
    
       for (int k = 0; k < fKriterien.length; k++)
          {
           zuFiltern = zuFiltern.stream().filter( fKriterien[k] ).collect( Collectors.toList() );
          }
       return zuFiltern.toArray(new Artikel[zuFiltern.size()]); 
     }
    */
    ...
    */
    public Artikel[] filterAll(Predicate<Artikel> ...fKriterien)
    {
      Stream<Artikel> streamZufiltern = lager.values().stream();
  
      for ( int k = 0; k < fKriterien.length; k++ )
         {
           streamZufiltern = streamZufiltern.filter(fKriterien[k]);
         }
      return streamZufiltern.toArray(Artikel[]::new);
    }
