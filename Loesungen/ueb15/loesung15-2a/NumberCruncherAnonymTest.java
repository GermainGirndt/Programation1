...
/**
 *    ----> NumberCruncherAnonymTest.java    --> Realisiert ein einfaches Dialogprogramm
 *                 zum testen der NumberCruncherAnonym-Klasse
 *
...
//------------------Konstanten----------------------------------
   ...
   //---------------- attribute --------------------
   private  NumberCruncherAnonym  nCr;


 /**
  *    Der Konstruktor
  */
  public NumberCruncherAnonymTest()
         throws Exception
  {
   ...
   int wahl  = MyInputFunctions.readlnInt( in,
    ...
    if ( wahl == TEST_MIT_ZUFALLS_ZAHLEN )
      {
       nCr = new NumberCruncherAnonym( dimension );
    ...
    else
      {
       if ( wahl == TEST_MIT_FESTEN_ZAHLEN )
         {
          nCr = new NumberCruncherAnonym( dimension, 2 );
          ...
          if ( wahl == TEST_MIT_EINGEGEBENEN_ZAHLEN )
            {
             ...
             nCr = new NumberCruncherAnonym( values );
...

   ...
   *    Das Test-Programm
   *
   *    Erzeugt ein NumberCruncherAnonymTest-Objekt
   *    und
   *    ruft dessen Start-Methode auf
   */
   public static void main ( String[] args )
   {
     NumberCruncherAnonymTest test;

     try
       {
        test = new NumberCruncherAnonymTest();
        test.start();
       }
     catch ( Exception e )
       {
